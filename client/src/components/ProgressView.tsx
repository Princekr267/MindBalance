  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const sorted = [...allAssessments].sort((a: any, b: any) => new Date(a.date).getTime() - new Date(b.date).getTime());
        ...
        if (allAssessments.length > 0) {
          const avg = allAssessments.reduce((sum: number, c: any) => sum + c.score, 0) / allAssessments.length;
      ...
      >
        ...
        {assessments.length === 0 ? (
          <div className="bg-white/10 backdrop-blur-lg rounded-3xl p-12 border border-white/20 text-center">
            ...
            <h3 className="text-white text-2xl mb-2">No Data Yet</h3>
          ...
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 mb-8 sm:mb-12">
              ...
              >
                <div className="text-white/60 mb-2">Total Assessments</div>
                <div className="text-white text-4xl">{assessments.length}</div>
              </motion.div>

              <motion.div
                className="bg-white/10 backdrop-blur-lg rounded-2xl p-6 border border-white/20"
              >
                <div className="text-white/60 mb-2">Avg Score</div>
                <div className="text-white text-4xl flex items-center gap-2">
                  {averageScore}
                  <span className="text-xl text-white/40">(Max 21)</span>
                </div>
              </motion.div>
              ...
              >
                ...
                <div className="text-white text-4xl flex items-center gap-2">
                  {trend === 'improving' ? (
                    ...
                    <span className="text-2xl text-white/60">Stable</span>
                  )}
                </div>
              </motion.div>
            </div>
            ...
            >
              <h3 className="text-white text-2xl mb-6">Score History</h3>
