  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const recentCheckIns = allCheckIns.filter((c: any) => new Date(c.timestamp).getTime() >= sevenDaysAgo);
        ...
        recentCheckIns.forEach((checkIn: any) => {
          ...
          dateMap.set(date, {
            total: existing.total + checkIn.stressScore,
        ...
        if (recentCheckIns.length > 0) {
          const avg = recentCheckIns.reduce((sum: number, c: any) => sum + c.stressScore, 0) / recentCheckIns.length;
